...
export const createPlatonicDescription = (index: number) => {
  ...
  DOMCacheGetOrSet('platonicUpgradeDescription').innerHTML = index === 20 && player.highestSingularityCount > 0 ?
  i18next.t('wowCubes.platonicCubes.upgradeDescriptions.platonic20Sing')
  : i18next.t(`wowCubes.platonicCubes.upgradeDescriptions.platonic${index}`)

  DOMCacheGetOrSet('platonicUpgradeLevel').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.upgradeLevel',
    {
  ...
  DOMCacheGetOrSet('platonicOfferingCost').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.offeringCost',
    {
  ...
  DOMCacheGetOrSet('platonicObtainiumCost').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.obtainiumCost',
    {
      a: format(player.researchPoints),
      b: format(platUpgradeBaseCosts[index].obtainium * priceMultiplier)
    }
  )
  DOMCacheGetOrSet('platonicCubeCost').textContent = i18next.t('wowCubes.platonicCubes.descriptionBox.cubeCost', {
  ...
  DOMCacheGetOrSet('platonicTesseractCost').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.tesseractCost',
    {
  ...
  DOMCacheGetOrSet('platonicHypercubeCost').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.hypercubeCost',
    {
  ...
  DOMCacheGetOrSet('platonicPlatonicCost').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.platonicCost',
    {
  ...
  DOMCacheGetOrSet('platonicHepteractCost').textContent = i18next.t(
    'wowCubes.platonicCubes.descriptionBox.hepteractCost',
    {
  ...
  if (player.platonicUpgrades[index] < platUpgradeBaseCosts[index].maxLevel) {
    ...
    if (resourceCheck.canBuy) {
      ...
      DOMCacheGetOrSet('platonicCanBuy').textContent = i18next.t(
        'wowCubes.platonicCubes.descriptionBox.platonicCanBuy'
      )
    } else {
      DOMCacheGetOrSet('platonicCanBuy').style.color = 'var(--crimson-text-color)'
      DOMCacheGetOrSet('platonicCanBuy').textContent = i18next.t(
        'wowCubes.platonicCubes.descriptionBox.platonicCannotBuy'
      )
    }
  }
  ...
  if (player.platonicUpgrades[index] === platUpgradeBaseCosts[index].maxLevel) {
    ...
    DOMCacheGetOrSet('platonicCanBuy').textContent = i18next.t(
      'wowCubes.platonicCubes.descriptionBox.platonicCanBuyMaxed'
    )
  }
}
  ...
  while (index > 0) {
    ...
    if (resourceCheck.canBuy) {
      ...
      if (index === 20 && !auto && player.singularityCount === 0) {
        void Alert(
          'While I strongly recommended you not to buy this, you did it anyway. For that, you have unlocked the Antiquities of Ant God, for you are a richass.'
        )
      }
    } else {
      break
